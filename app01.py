...
# pip install Flask Flask-SQLAlchemy flask-cors psycopg2-binary gunicorn
...
def get_dashboard_stats():
    try:
        ...
        tecnico_counts_query = base_query.join(Tecnico, Chamado.tecnico_id == Tecnico.id, isouter=True).with_entities(Tecnico.nome, func.count(Chamado.id)).group_by(Tecnico.nome)
        tecnico_counts = tecnico_counts_query.all()
        chamados_por_tecnico = {nome if nome else 'Não Atribuído': count for nome, count in tecnico_counts}
