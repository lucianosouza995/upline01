...
def get_dashboard_stats():
    try:
        ...
        chamados_mes = db.session.query(extract('year', Chamado.timestamp).label('ano'), extract('month', Chamado.timestamp).label('mes'), func.count(Chamado.id)).filter(Chamado.timestamp >= doze_meses_atras).group_by('ano', 'mes').order_by('ano', 'mes').all()
        chamados_por_mes = [{'mes': f"{int(mes):02d}/{int(ano)}", 'total': total} for ano, mes, total in chamados_mes]
        return jsonify({
            'total_chamados': Chamado.query.count(), 'total_tecnicos': Tecnico.query.count(), 'total_elevadores': Elevador.query.count(),
